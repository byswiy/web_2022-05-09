...
public class ProductInfoUpdateController1 extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		try {
			...
			if(!validator.nameValidator(name)) 				throw new BadParameterException1();
			else if(!validator.categoryValidator(category))	throw new BadParameterException1();
			else if(!validator.stockValidator(stock)) 		throw new BadParameterException1();
			else if(!validator.priceValidator(price))		throw new BadParameterException1();
